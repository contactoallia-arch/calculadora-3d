...
export default async function handler(req, res) {
  ...
  // POST: ejecutar consulta SQL personalizada (solo lectura)
  if (req.method === "POST") {
    const { sql } = req.body || {};
  ...
  // GET sin tabla: listar todas las tablas con conteo de filas
  if (!table) {
  ...
  // GET con tabla: esquema + datos paginados
  ...
  // Conteo total (con o sin filtro)
  const countArgs = [];
  let countSql = `SELECT COUNT(*) as c FROM "${table}"`;
  if (search && col) {
    countSql += ` WHERE CAST("${col}" AS TEXT) LIKE ?`;
  ...
  if (search && col) {
    dataSql += ` WHERE CAST("${col}" AS TEXT) LIKE ?`;
    dataArgs.push(`%${search}%`);
  }
